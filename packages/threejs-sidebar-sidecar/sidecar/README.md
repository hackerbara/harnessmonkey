...
For HarnessMonkey manual smoke, either enable the `threejs-sidebar-sidecar-local` option package or set:

```bash
export CLAUDEMONKEY_THREE_SIDECAR_ENABLE=1
export CLAUDEMONKEY_THREE_SIDECAR=~/.harnessmonkey/patches/threejs-sidebar-sidecar/sidecar/run-sidecar.js
